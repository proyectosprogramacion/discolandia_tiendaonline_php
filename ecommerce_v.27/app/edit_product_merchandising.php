...
if (isset($_GET['id'])) {
  ...
  while ($row = fetch_array($query)) {
    ...
    $product_title = escape_string($row['merchand_title']);
    $product_category_id = escape_string($row['merchand_category_id']);
    $product_price = escape_string($row['merchand_price']);
    $product_description = escape_string($row['merchand_description']);
    $short_desc = escape_string($row['merchand_short_desc']);
    $product_quantity = escape_string($row['merchand_quantitiy']);
    $product_image = escape_string($row['merchand_image']);



    $product_image = display_image($row['merchand_image']);

  }

  update_product_merchandising();
