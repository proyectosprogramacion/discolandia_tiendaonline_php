...
function products_count()
{
    ...
    $result = query("SELECT count(*) as total from merchandising");
    $data_merchandising = mysqli_fetch_assoc($result);

    echo $data_merchandising['total'] + $data['total'];
...
{
    $result = query("SELECT count(*) as total from categories");
    $data = mysqli_fetch_assoc($result);
    echo $data['total'];
    ...
    while ($row = fetch_array($query)) {
        $orders = <<<DELIMETER
            ...
            <tr>
                ...
                <td><a href="admin.php?edit_order&id={$row['order_transaction_id']}">{$row['order_status']}</a><br>
...
function get_products_merchandising()
{
    ...
    while ($row = fetch_array($query)) {
        ...
        $merchandising_image = display_image($row['merchand_image']);

        $merchandising = <<<DELIMETER

        <tr>
            <td>{$row['merchand_id']}</td>
            <td><a href="admin.php?edit_product_merchandising&id={$row['merchand_id']}">{$row['merchand_title']}</a><br>
                 <img width='100' src="{$merchandising_image}" alt="">
            </td>
            <td>{$row['merchand_description']}</td>
            <td>{$row['merchand_short_desc']}</td>
            <td>{$category}</td>
            <td>{$row['merchand_price']}</td>
            <td>{$row['merchand_quantitiy']}</td>
             <td><a class="btn btn-primary" href="admin.php?delete_product_merchandising_id={$row['merchand_id']}"><span class="glyphicon glyphicon-remove"></span></a></td>
        </tr>
...
function update_order()
{
    if (isset($_POST['update_order'])) {
        ...
        $order_transaction_id  = escape_string($_POST['order_transaction_id']);
        ...
        $query .= "order_transaction_id = '{$order_transaction_id}'        , ";
        $query .= "order_quantity      = '{$order_quantity}'  , ";
        $query .= "order_amount            = '{$order_amount}'        , ";
        $query .= "order_status   = '{$order_status}'  , ";
    
        $query .= "WHERE order_transaction_id=". escape_string($_GET['id']);
...
function update_product_merchandising(){
    if (isset($_POST['update'])) {
        ...
        if (empty($product_image)) {
            ...
            $get_pic = query("SELECT merchand_image  FROM merchandising WHERE  	merchand_id =" . escape_string($_GET['id']) . " ");
            confirm($get_pic);

            while ($pic = fetch_array($get_pic)) {

                $product_image = $pic['merchand_image'];
        ...
        $query .= "merchand_title            = '{$product_title}'        , ";
        $query .= "merchand_category_id      = '{$product_category_id}'  , ";
        $query .= "merchand_price          = '{$product_price}'        , ";
        $query .= "merchand_description      = '{$product_description}'  , ";
        $query .= "merchand_short_desc          = '{$short_desc}'           , ";
        $query .= "merchand_quantitiy        = '{$product_quantity}'     , ";
        $query .= "merchand_image          = '{$product_image}'          ";
        $query .= "WHERE merchand_id=" . escape_string($_GET['id']);
...
function add_product_merchandising()
{
    if (isset($_POST['publish'])) {
        ...
        $query = query("INSERT INTO merchandising (merchand_title, merchand_category_id, merchand_price, merchand_description, merchand_short_desc, merchand_quantitiy, merchand_image ) VALUES('{$product_title}', '{$product_category_id}', '{$product_price}', '{$product_description}', '{$short_desc}', '{$product_quantity}', '{$product_image}')");
        $last_id = last_id();
        confirm($query);
        set_message("Nuevo producto con id {$last_id} ha sido añadido");
        redirect("admin.php?products_merchandising");
